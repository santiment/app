import Web3 from 'web3'; // let prevSelectedAccount = null
...
export const getAccount = () => new Promise(async (resolve, reject) => {
  // Modern dapp browsers
  if (window.ethereum) {
    try {
    ...
    setInterval(() => {
      const accounts = localWeb3.eth.accounts; // if (prevSelectedAccount !== selectedAccount) {
      // prevSelectedAccount = selectedAccount

      resolve(accounts[0]); // }
    }, 100); // resolve(new Web3(web3.currentProvider)) // eslint-disable-line
  }
