/* eslint-env jest */
import { formatCryptoCurrency, formatBTC, formatSAN, formatNumber, millify, upperCaseFirstLetter } from './formatting';
describe('formatCryptoCurrency', () => {
  it('returns properly formated string', () => {
    expect(formatCryptoCurrency('BTC', '50.123456789')).toEqual('BTC 50.123456789');
  });
});
describe('formatBTC', () => {
...
describe('formatNumber', () => {
  it('throws an error if input is not supported', () => {
    ...
    expect(() => formatNumber({
      foo: 'bar'
    })).toThrowError('Unsupported type: "[object Object]"');
  });
  it('parses input to float', () => {
...
describe('upperCaseFirstLetter', () => {
  it('word in lowercase', () => expect(upperCaseFirstLetter('word')).toEqual('Word'));
  it('word with uppercased first letter', () => expect(upperCaseFirstLetter('Word')).toEqual('Word'));
  it('two words', () => expect(upperCaseFirstLetter('first LeTteR')).toEqual('First LeTteR'));
  it('without letters', () => expect(upperCaseFirstLetter('!!!')).toEqual('!!!'));
});