...
`);
export const useOldUserFollowersFollowing = ({
  userId,
  username
}) => {
...
export const updateFollowersList = (followers, follow, unfollow, userId) => {
  const isInList = followers.users.some(({
    id
  }) => +id === +userId);
  const {
    users
...
export const updateFollowingList = (usersList, userData) => {
  const isInList = usersList.users.some(({
    id
  }) => +id === +userData.id);
  const {
    users
...
export const updateCurrentUserFollowQueryCache = (cache, {
  data: {
    follow,
    unfollow
  }
}, queryVariables, userId, followingUser, currentUserId) => {
  update({
    queryVariables,
    follow,
    unfollow,
...
const update = ({
  ...
  const data = cloneDeep(cache.readQuery({
