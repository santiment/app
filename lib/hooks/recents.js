...
export function useRecentAssets(slugs) {
  ...
  useEffect(() => {
    ...
    let assets = [];
    let race = false;
    Promise.all(slugs.map((id, i) => getRecentAsset(id).then(asset => assets[i] = asset))).then(data => {
      if (race) return;
...
export function useRecentTemplates(templatesIDs) {
  ...
  useEffect(() => {
    ...
    let templates = [];
    let race = false;
    Promise.all(templatesIDs.map((id, i) => getRecentTemplate(id).then(template => templates[i] = template))).then(data => {
      if (race) return;
