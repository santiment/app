...
export function useUsernameChange(_username) {
  ...
  const changeUsername = username => mutate({
    variables: {
      username
    }
  });
...
export function useFullnameChange(_fullname) {
  ...
  const [mutate, {
    loading
  }] = useMutation(CHANGE_NAME_MUTATION);
  ...
  const changeFullname = fullname => mutate({
    variables: {
      fullname
    }
  });
  ...
  const showFullnameChangedNotifiction = (notification = null, _fullname) => {
    if (notification) {
      ...
      store.dispatch(showNotification(`Full name successfully changed to "${_fullname || fullname}"`));
    }
  };
