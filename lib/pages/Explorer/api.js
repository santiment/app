...
export const queryExplorerItems = ({
  ...
  if (voted) QUERYKEY = QUERYKEY.concat('Voted');else if (favorites) QUERYKEY = QUERYKEY.concat('Used');else QUERYKEY = QUERYKEY.concat('Recent');
  ...
  const projectWatchlist = TYPES.has(EntityKeys.PROJECT_WATCHLIST) ? EntityQuery.projectWatchlist : '';
  const screener = TYPES.has(EntityKeys.SCREENER) ? EntityQuery.screener : '';
  const chartConfiguration = TYPES.has(EntityKeys.CHART_CONFIGURATION) ? EntityQuery.chartConfiguration : '';
  const insight = TYPES.has(EntityKeys.INSIGHT) ? EntityQuery.insight : '';
  const addressWatchlist = TYPES.has(EntityKeys.ADDRESS_WATCHLIST) ? EntityQuery.addressWatchlist : '';
