import React, { useState } from 'react';
import cx from 'classnames';
import { connect } from 'react-redux';
import Icon from '@santiment-network/ui/Icon';
import Label from '@santiment-network/ui/Label';
import Button from '@santiment-network/ui/Button';
import copy from 'copy-to-clipboard';
import Settings from './Settings';
import * as actions from '../../actions/types';
import ApiCallsStatistic, { API_KEYS_STATS } from './ApiCallsStatistic';
import accountStyles from './AccountPage.module.css';
import styles from './SettingsAPIKeys.module.css';
let genTimer;

const SettingsAPIKeys = ({
  apikeys = [],
  generateAPIKey,
  revokeAPIKey
}) => {
  const [copiedShown, setCopiedShown] = useState('');

  function showCopiedTooltip(apikey) {
    setCopiedShown(apikey);
    setTimeout(() => setCopiedShown(''), 1000);
  }

  function onGenClick() {
    clearTimeout(genTimer);
    genTimer = setTimeout(generateAPIKey, 300);
  }

  return /*#__PURE__*/React.createElement(Settings, {
    id: "api-keys",
    header: "API keys"
  }, /*#__PURE__*/React.createElement(Settings.Row, {
    className: styles.descriptionRow
  }, /*#__PURE__*/React.createElement(Label, {
    className: cx(accountStyles.label),
    accent: "waterloo"
  }, /*#__PURE__*/React.createElement("p", null, "The api key can only be used to fetch data and not to execute graphql mutations.", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("br", null), "You can try out queries through our", ' ', /*#__PURE__*/React.createElement("a", {
    href: "https://api.santiment.net/graphiql?query=%7B%0A%20%20networkGrowth(from%3A%20%222019-05-09T11%3A25%3A04.894Z%22%2C%20interval%3A%20%221d%22%2C%20slug%3A%20%22ethereum%22%2C%20to%3A%20%222019-06-23T11%3A25%3A04.894Z%22)%20%7B%0A%20%20%20%20newAddresses%0A%20%20%20%20datetime%0A%20%20%7D%0A%7D%0A&variables=%7B%7D",
    target: "_blank",
    rel: "noopener noreferrer",
    className: styles.link
  }, "web explorer"), ' ', "or explore our", ' ', /*#__PURE__*/React.createElement("a", {
    href: "https://academy.santiment.net/sanapi/",
    target: "_blank",
    rel: "noopener noreferrer",
    className: styles.link
  }, "documentation"), ' ', "to get started.")), apikeys.length > 0 ? apikeys.map(apikey => /*#__PURE__*/React.createElement("div", {
    key: apikey,
    className: styles.keyContainer
  }, /*#__PURE__*/React.createElement("div", {
    className: cx(styles.apikey, copiedShown === apikey && styles.copied)
  }, /*#__PURE__*/React.createElement("input", {
    className: styles.apikey__input,
    defaultValue: apikey,
    readOnly: true
  }), /*#__PURE__*/React.createElement(Icon, {
    onClick: () => {
      copy(apikey);
      showCopiedTooltip(apikey);
    },
    type: "copy",
    className: styles.apikey__icon
  })), /*#__PURE__*/React.createElement(Button, {
    onClick: () => {
      revokeAPIKey(apikey);
    },
    accent: "negative"
  }, "Revoke"))) : /*#__PURE__*/React.createElement(Button, {
    onClick: onGenClick,
    variant: "fill",
    accent: "positive",
    className: styles.generateButton
  }, "Generate")), /*#__PURE__*/React.createElement(ApiCallsStatistic, {
    type: API_KEYS_STATS.APIKEY
  }));
};

const mapStateToProps = ({
  user: {
    data: {
      apikeys = []
    }
  }
}) => ({
  apikeys
});

const mapDispatchToProps = dispatch => ({
  generateAPIKey: () => dispatch({
    type: actions.USER_APIKEY_GENERATE
  }),
  revokeAPIKey: apikey => dispatch({
    type: actions.USER_APIKEY_REVOKE,
    apikey
  })
});

export default connect(mapStateToProps, mapDispatchToProps)(SettingsAPIKeys);