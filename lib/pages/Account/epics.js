...
export const connectTelegramEpic = (action$, store, {
  client
}) => action$.ofType(actions.SETTINGS_CONNECT_TELEGRAM).mergeMap(action => {
  const telegramConnection$ = Observable.from(client.watchQuery({
    query: USER_SETTINGS_TELEGRAM_QUERY,
    pollInterval: 2000
  })).takeUntil(action$.ofType(actions.SETTINGS_CONNECT_TELEGRAM_CANCEL)).filter(({
    data
  }) => {
  ...
  const timeout$ = Observable.timer(TIMEOUT_DELAY).takeUntil(action$.ofType(actions.SETTINGS_CONNECT_TELEGRAM_SUCCESS)).mapTo({
    type: actions.SETTINGS_CONNECT_TELEGRAM_CANCEL
  });
  return Observable.merge(telegramConnection$, timeout$);
});
export const revokeTelegramDeepLinkEpic = (action$, store, {
  client
...
export const generateTelegramDeepLinkEpic = (action$, store, {
  client
  ...
  return Observable.from(getTelegramDeepLink).mergeMap(({
    data
  }) => {
