...
import { queryLayout } from 'san-studio/lib/api/layouts';
import { selectedLayout } from 'san-studio/lib/stores/layout';
import Studio from './Studio';
import URLExtension from './URLExtension';
import RecentAssetExtension from './RecentAssetExtension';
import { SHORT_URL_POSTFIX, getShortUrlHash, onAnonComment, handleSavedComment, handleLayoutCommentLink, onDefaultLayoutAddressSelect } from './utils';
...
const Extensions = props => /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(URLExtension, props), /*#__PURE__*/React.createElement(RecentAssetExtension, {
  settings: props.settings
}), /*#__PURE__*/React.createElement(CtaJoinPopup, null));
...
export default (({
  ...
  useEffect(() => {
    if (isLoggedIn && layout && layout.id) {
      ...
      window.onCommentSubmitted = () => mutateStoreUserActivity(EntityKeys.CHART_CONFIGURATION, layout.id, InteractionType.COMMENT);
    }
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(templateId)) {
      ...
      queryLayout(+templateId).then(layout => {
        if (isRacing) return;
        const parsedUrl = {
    ...
    getFullUrl(shortUrlHash).then(fullUrl => {
      if (isRacing) return;
  ...
  if (!parsedUrl) return /*#__PURE__*/React.createElement(PageLoader, null);
  const {
    widgets,
    settings,
    sidewidget
  ...
  if (!isDesktop) {
    return /*#__PURE__*/React.createElement(Redirect, {
      to: '/projects/' + (settings ? settings.slug : slug)
    });
  }
