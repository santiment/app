...
const {
  from: ANON_LEFT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-3, MONTH, correctFromByHours(THREE_DAYS_IN_HOURS));
const {
  from: ANON_RIGHT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-1, DAY, correctFromByHours(ONE_DAY_IN_HOURS));
const {
  from: FREE_LEFT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-24, MONTH, correctFromByHours());
const {
  from: FREE_RIGHT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-1, MONTH, correctFromByHours());
const {
  from: BASIC_LEFT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-24, MONTH, correctFromByHours());
const {
  from: BASIC_RIGHT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-7, DAY, correctFromByHours());
const {
  from: PRO_LEFT_BOUNDARY_DATE
} = getTimeIntervalFromToday(-60, MONTH, correctFromByHours());
