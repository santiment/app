...
export const useRawSignals = ({
  ...
  return useMemo(() => {
    ...
    return {
      data: data ? data.getRawSignals.filter(signal => signal && (!!signal.project || signal.isHidden)) : ARRAY,
      loading: loading || isLoading
    };
  }, [data, loading, isLoading]);
};

const marketcapSorter = groups => (a, b) => groups[b].project.marketcapUsd - groups[a].project.marketcapUsd;

export function useGroupedBySlugs(signals, hiddenLabels, selectedAssets) {
  const filteredByAssets = useMemo(() => signals.filter(({
    slug
  }) => selectedAssets[slug]), [signals, selectedAssets]);
  const {
    slugs,
    projects
  } = useMemo(() => ({
    slugs: [...new Set(signals.map(({
      slug
    }) => slug))],
  ...
  const restrictedSignals = useMemo(() => signals.filter(({
    isHidden
  }) => isHidden).map(({
    signal
  }) => signal), [signals]);
  const labels = useMemo(() => {
    const signalNames = [...filteredByAssets.map(({
      signal
    }) => signal), ...restrictedSignals];
  ...
  const {
    ...
    const visibleSlugs = Object.keys(groups).filter(slug => !!groups[slug].project).sort(marketcapSorter(groups));
    return {
      groups,
      visibleSlugs
    };
