...
import { USER_TOGGLE_BETA_MODE, APP_USER_BETA_MODE_SAVE, APP_USER_BETA_MODE_SAVE_FAILED, CHANGE_USER_DATA } from '../actions/types';
...
const handleBetaModeToggle = (action$, store, {
  client
}) => action$.ofType(USER_TOGGLE_BETA_MODE).map(() => {
...
export const saveBetaModeAfterLaunch = action$ => action$.ofType(CHANGE_USER_DATA).filter(({
  user = {}
}) => user.settings && user.settings.isBetaMode).filter(() => loadKeyState('isBetaMode') === undefined).mergeMap(() => {
...
export const sendBetaModeIfDiff = (action$, store, {
  client
}) => action$.ofType(CHANGE_USER_DATA).filter(() => loadKeyState('isBetaMode') === undefined && loadKeyState('isBetaModeEnabled') === true).mergeMap(({
  user: {
    settings: {
      isBetaMode
    }
