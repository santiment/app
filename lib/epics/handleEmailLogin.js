...
import { trackTwitterSignUpEvent } from 'san-webkit/lib/analytics/twitter';
...
import { USER_GQL_FRAGMENT } from './handleLaunch'; // import { trackSignupLogin } from '../firstLogin'
...
`;
export const handleLoginSuccess = action$ => action$.ofType(actions.USER_LOGIN_SUCCESS).mergeMap(action => {
  const {
    token,
    consent,
    user
  ...
  window.$FPROM && window.$FPROM.trackSignup({
    email // TODO: add this after we will have this info --> uid:<user-billing-id>
  ...
  } = getSavedLoginMethod() || {}; // trackSignupLogin(isFirstLogin, method)
  ...
  return Observable.merge(Observable.of(showNotification('You are logged in!')), consent ? Observable.of(replace(`/consent?consent=${consent}&token=${token}`)) : Observable.empty());
}).catch(error => {
...
const handleEmailLogin = (action$, store, {
  client
}) => action$.ofType(actions.USER_EMAIL_LOGIN).takeUntil(action$.ofType(actions.USER_LOGIN_SUCCESS)).switchMap(action => {
  setCoupon(action.payload.coupon);
  const mutationGQL = action.payload.email ? EMAIL_LOGIN_VERIFY_MUTATION : EMAIL_CHANGE_VERIFY_MUTATION;
