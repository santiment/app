  ...
  for (let i = 0; i < length; i++) {
    ...
    if (metric && AvailableTimeboundMetric[key]) {
      ...
      if (!timeboundMetric) {
        ...
        timeboundMetric = _objectSpread(_objectSpread({}, metric), {}, {
          queryKey: withoutRoot ? timeboundKey : queryKey,
          label: metric.label + ` (${timebound})`,
          fullTitle: metric.fullTitle ? metric.fullTitle + ` (${timebound})` : undefined,
          key: timeboundKey,
          replacements: {
            timebound
          }
...
export function getMergedTimeboundSubmetrics(metricKeys) {
  ...
  const setOfKeys = new Set(Object.keys(Timebounds).concat(Object.keys(Submetrics)));
...
export const useMergedTimeboundSubmetrics = metricKeys => useMemo(() => getMergedTimeboundSubmetrics(metricKeys), [metricKeys]);
...
function timeboundsSorter({
  ...
  const {
    amount: aAmount,
    format: aFormat
  } = parseIntervalString(getTimerange(aKey));
  const {
    amount: bAmount,
    format: bFormat
  } = parseIntervalString(getTimerange(bKey));
  return aAmount * TimerangeCoeficient[aFormat] - bAmount * TimerangeCoeficient[bFormat];
}