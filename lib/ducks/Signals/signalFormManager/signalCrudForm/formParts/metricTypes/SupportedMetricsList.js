...
export function filterOnlyMetrics(submetrics) {
  ...
  Object.keys(submetrics).forEach(key => {
    result[key] = submetrics[key].filter(({
      type
    }) => !type || type === METRIC);
  });
  return result;
...
const getByAvailable = (availableMetrics = DEFAULT_METRICS, trigger) => {
  ...
  if (watchlist_id) {
    ...
    return SIGNAL_SUPPORTED_METRICS.filter(({
      key
    }) => {
...
export function useAvailableMetrics(slug) {
  const {
    ...
    loading,
    error
  } = useQuery(PROJECT_METRICS_BY_SLUG_QUERY, {
    skip: !slug,
    variables: {
      slug
    }
...
const SupportedMetricsList = ({
  ...
  const metrics = useMemo(() => {
    return getByAvailable(availableMetrics.some(m => m.includes('nvt')) ? availableMetrics.concat('nvt_5min') : availableMetrics, trigger);
  ...
  return categoriesKeys.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: metricStyles.choose
  }, /*#__PURE__*/React.createElement("div", {
    className: metricStyles.chooseText
  }, "or choose from the group of metrics"), /*#__PURE__*/React.createElement("div", {
    className: metricStyles.divider
    ...
    className: metricStyles.metrics
  }, categoriesKeys.map(key => /*#__PURE__*/React.createElement(MetricsList, {
    key: key,
    metrikKey: key,
    list: categories[key],
