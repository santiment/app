...
export const isWebhookChannel = channel => {
  return typeof channel === 'object' && (channel.hasOwnProperty('webhook') || Object.keys(channel).length === 0);
};
...
export const useChannelTypes = ({
  ...
  const isActive = useCallback(channel => {
    return isActiveInChannels(channels, channel);
  }, [channels]);
  const isRequired = useCallback(channel => {
    return requiredChannels.some(required => required === channel) || disabledChannels.some(disabled => disabled === channel);
  }, [requiredChannels, disabledChannels]);
  const isDisabled = useCallback(channel => {
