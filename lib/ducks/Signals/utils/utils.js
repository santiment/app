...
import { ETH_WALLETS_OPERATIONS, ETH_WALLET_METRIC, REQUIRED_MESSAGE, PRICE_PERCENT_CHANGE_UP_MODEL, PRICE_PERCENT_CHANGE, PRICE_VOLUME_DIFFERENCE, PRICE_ABSOLUTE_CHANGE_DOUBLE_BORDER, PRICE_ABSOLUTE_CHANGE_SINGLE_BORDER, PRICE_ABSOLUTE_CHANGE, DAILY_ACTIVE_ADDRESSES, PRICE_CHANGE_TYPES, FREQUENCY_TYPE_ONCEPER_MODEL, FREQUENCY_TYPES_OPTIONS, MUST_BE_MORE_ZERO_MESSAGE, PRICE_PERCENT_CHANGE_DOWN_MODEL, ETH_WALLET, DAILY_ACTIVE_ADDRESSES_METRIC, PRICE_VOLUME_DIFFERENCE_METRIC, PRICE_METRIC, PRICE_ABS_CHANGE_OUTSIDE, PRICE_ABS_CHANGE_INSIDE, PRICE_ABS_CHANGE_BELOW, PRICE_ABS_CHANGE_ABOVE, COOLDOWN_TYPES, COOLDOWN_REGEXP, FREQUENCY_MAPPINGS, FREQUENCY_VALUES, BASE_THRESHOLD, BASE_PERCENT_THRESHOLD, PREVIEWS_TIMERANGE_BY_TYPE, TIME_WINDOW_UNITS, getDefaultTimeRangeValue, TRENDING_WORDS_METRIC, TRENDING_WORDS_PROJECT_MENTIONED, TRENDING_WORDS, TRENDING_WORDS_WORD_MENTIONED, METRIC_TARGET_ASSETS, METRIC_TARGET_WATCHLIST, METRIC_TARGET_TEXT, TRENDING_WORDS_WATCHLIST_MENTIONED, PRICE, METRIC_DEFAULT_VALUES, NOT_VALID_HB_ADDRESS, MIN_TITLE_LENGTH, MAX_TITLE_LENGTH, MAX_DESCR_LENGTH, PRICE_PERCENT_CHANGE_ONE_OF_MODEL, CHANNELS_MAP, POSSIBLE_METRICS_FOR_CHART, SIGNAL_METRIC_TYPES, METRIC_TYPES, AMOUNT_ABS_CHANGE_UP_MODEL, AMOUNT_ABS_CHANGE_DOWN_MODEL, PRICE_ABS_CHANGE_BELOW_OR_EQUAL, PRICE_ABS_CHANGE_ABOVE_OR_EQUAL } from './constants';
import { capitalizeStr } from '../../../utils/utils';
import { formatNumber } from '../../../utils/formatting';
import { Metric } from '../../dataHub/metrics';
import { useWatchlist } from '../../Watchlists/gql/hooks';
import { isDailyMetric, SIGNAL_SUPPORTED_METRICS } from '../signalFormManager/signalCrudForm/formParts/metricTypes/metrics';
...
export const targetMapperWithName = ({
  value,
  slug,
  name
} = {}) => name || slug || value;
export const targetMapperWithTicker = ({
  value,
  slug,
  ticker
} = {}) => ticker || slug || value;
...
const getFormTriggerTarget = settings => {
  ...
  if (selector || address) {
    const newEthAddress = address ? Array.isArray(address) ? mapToOptions(address) : mapToOptions([address]) : undefined;
    return {
    ...
    case METRIC_TYPES.METRIC_SIGNAL:
      {
        switch (metric) {
          ...
          default:
            {
              return SIGNAL_SUPPORTED_METRICS.find(({
                key
              }) => key === metric) || makeSubmetric(metric);
            }
        }
      }
...
const mapTriggerToFormThreshold = ({
  threshold,
  operation
}) => {
...
export const mapFormPropsToScreenerTrigger = ({
  formProps,
  signal
}) => {
  const {
  ...
  const trigger = _objectSpread(_objectSpread(_objectSpread({}, SCREENER_DEFAULT_SIGNAL), signal), {}, {
    description,
    title,
    isActive,
    cooldown
...
export const extractTimeWindowForm = time_window => {
  return {
    ...
    timeWindowUnit: time_window ? getTimeWindowUnit(time_window) : TIME_WINDOW_UNITS[0]
  };
};
export const mapTriggerToFormProps = currentTrigger => {
  ...
  return _objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread({
    ...
    channels: Array.isArray(channel) ? channel.map(mapToFormChannel) : [mapToFormChannel(channel)]
  }, frequencyModels), absolutePriceValues), trendingWordsParams), {}, {
    title: clipText(Array.isArray(title) ? title.join(' ') : title, MAX_TITLE_LENGTH),
...
const getPercentTreshold = ({
  ...
  const parsedThreshold = operation ? operation[Object.keys(operation)[0]] : BASE_PERCENT_THRESHOLD;

  if (newType && newType.value === PRICE_CHANGE_TYPES.PERCENT_SOME_OF) {
    return {
      percentThresholdLeft: operation[PRICE_CHANGE_TYPES.PERCENT_SOME_OF][0][PRICE_CHANGE_TYPES.MOVING_UP] || BASE_PERCENT_THRESHOLD,
      percentThresholdRight: operation[PRICE_CHANGE_TYPES.PERCENT_SOME_OF][1][PRICE_CHANGE_TYPES.MOVING_DOWN] || BASE_PERCENT_THRESHOLD
    };
  } else {
    return {
      percentThreshold: parsedThreshold || percentThreshold || BASE_PERCENT_THRESHOLD
    };
  }
};

const getCooldownParams = ({
  frequencyTimeType,
  frequencyTimeValue
}) => {
...
const getFrequencyFromCooldown = ({
  cooldown
}) => {
...
export const getTargetFromArray = (target, mapper = targetMapper) => target.length === 1 ? mapper(target[0]) : target.map(mapper);
export const mapFormTargetToTriggerTarget = (target, targetWatchlist, textSelector, signalType = {}, address) => {
  const {
    value
  } = signalType;
    ...
    case METRIC_TARGET_TEXT.value:
      {
        return {
          target: {
            text: textSelector ? mapTargetObject(textSelector) : ''
          }
        };
      }
    ...
    default:
      {
        if (address) {
          return {
            target: {
              address: mapTargetObject(address)
            }
          };
        } else {
          return {
...
export const mapTargetObject = (target, mapper = targetMapper) => {
  return Array.isArray(target) ? getTargetFromArray(target, mapper) : mapper(target);
};
...
const mapToFormChannel = channelValue => {
  ...
  return CHANNELS_MAP.find(({
    value
  }) => value === channelValue.toLowerCase()).label;
};
...
export const isTrendingWordsByProjects = type => type.value === TRENDING_WORDS_PROJECT_MENTIONED.value;
export const isTrendingWordsByWords = type => type.value === TRENDING_WORDS_WORD_MENTIONED.value;
export const isTrendingWordsByWatchlist = type => type.value === TRENDING_WORDS_WATCHLIST_MENTIONED.value;
export const getTrendingWordsTriggerOperation = ({
  type: {
    value
  },
  type
...
export const getTrendingWordsTarget = ({
  ...
  switch (type.value) {
    ...
    case TRENDING_WORDS_WATCHLIST_MENTIONED.value:
      {
        return {
          watchlist_id: targetWatchlist ? +(targetWatchlist.id || targetWatchlist.value) : undefined
        };
      }
...
export const mapFormToPPCTriggerSettings = formProps => {
  ...
  const newTarget = mapFormTargetToTriggerTarget(target, targetWatchlist, textSelector, signalType);
...
export const mapFormToHBTriggerSettings = formProps => {
  ...
  const newTarget = mapFormTargetToTriggerTarget(target, targetWatchlist, textSelector, signalType, ethAddress);
  const newAsset = mapHBAssetTarget({
    target
  });
...
export const mapFormPropsToTrigger = (formProps, prevTrigger) => {
  ...
  if (metric) {
    switch (metric.value) {
      ...
      default:
        {
          settings = mapDefaultMetricProps({
            type,
            formProps
          });
        }
  ...
  return _objectSpread(_objectSpread(_objectSpread({}, prevTrigger), {}, {
    settings,
    isRepeating: !!isRepeating
  }, cooldownParams), {}, {
    isPublic: !!isPublic,
...
const HistoricalBalanceMetrics = _objectSpread(_objectSpread({}, Metric.balance), {}, {
  key: 'historicalBalance',
  queryKey: 'historicalBalance'
});

export const getNewMetricsByType = ({
  settings: {
    type,
    metric
  }
...
export const isAsset = signalType => signalType.value === METRIC_TARGET_ASSETS.value;
export const isWatchlist = signalType => signalType.value === METRIC_TARGET_WATCHLIST.value;
export const isText = signalType => signalType.value === METRIC_TARGET_TEXT.value;
export const validateTriggerForm = values => {
  const errors = _objectSpread(_objectSpread(_objectSpread(_objectSpread({}, metricTypesBlockErrors(values)), metricValuesBlockErrors(values)), descriptionBlockErrors(values)), validateChannels(values));
...
export const metricTypesBlockErrors = values => {
  ...
  if (metric && metric.value === ETH_WALLET) {
    if (hasHBAddresses(ethAddress)) {
      ...
      if (isHbAddressError && (Array.isArray(target) ? target.length === 1 : !!target)) {
        errors.ethAddress = NOT_VALID_HB_ADDRESS;
      }
    }
    ...
    if (isTrendingWordsByWords(type) && (!trendingWordsWithWords || trendingWordsWithWords.length === 0)) {
    ...
    if (isWatchlist(signalType)) {
      ...
      if (!target || (Array.isArray(target) ? target.length === 0 : !target.value)) {
        errors.target = REQUIRED_MESSAGE;
      }
    }
  }
...
export const metricValuesBlockErrors = values => {
  ...
  if (type.metric === PRICE_PERCENT_CHANGE) {
    if (type.dependencies) {
      ...
      errors = checkByKey(errors, 'percentThresholdLeft', values, type.dependencies);
      errors = checkByKey(errors, 'percentThresholdRight', values, type.dependencies);
    }
  }
...
export const couldShowChart = (settings, types = POSSIBLE_METRICS_FOR_CHART) => {
  const {
    target = {},
  ...
  switch (checking) {
    ...
    case ETH_WALLET:
      {
        const checkedSelector = selector ? (selector.currency || selector.slug) && selector.infrastructure : true;
        return checkedSelector && Array.isArray(ethAddress) ? ethAddress.length === 1 : !!ethAddress;
      }
...
export const mapToAssets = (data, withFilter = true) => {
  ...
  return data.filter(asset => !withFilter || !!asset.mainContractAddress).map((asset, index) => {
...
export const getDefaultFormValues = (newValues, {
  ...
  return _objectSpread(_objectSpread({}, defaultValues), newValues);
};
...
const targetsJoin = targets => Array.isArray(targets) ? targets.join(', ') : targets;

export const hasHBAddresses = ethAddress => Array.isArray(ethAddress) ? ethAddress.length > 0 : !!ethAddress;
  ...
  if (metric) {
    if (metric.value === TRENDING_WORDS) {
      switch (type.value) {
        ...
        case TRENDING_WORDS_WORD_MENTIONED.value:
          {
            const targets = mapTargetObject(trendingWordsWithWords, targetMapperWithName);
            return buildFormBlock(NOTIFY_ME_WHEN, targetsJoin(targets));
          }

        case TRENDING_WORDS_WATCHLIST_MENTIONED.value:
          {
            return buildFormBlock(NOTIFY_ME_WHEN, targetMapperWithName(targetWatchlist));
          }
      ...
      if (Array.isArray(ethAddress) && ethAddress.length > 1) {
        return buildFormBlock(NOTIFY_ME_WHEN, `${targetsJoin(targets)} wallets [${addresses.join(', ')}]`);
      } else {
        const walletDescription = addresses ? 'wallet ' + addresses : '';
        return buildFormBlock(NOTIFY_ME_WHEN, `${targetsJoin(targets)} ${walletDescription}`);
      }
    }
  }
...
const getUsd = (value = 0, isPriceMetric) => isPriceMetric ? formatNumber(value || 0, {
  currency: 'USD'
}) : value;
...
export const titleMetricValuesHeader = (hasMetricValues, {
  type,
  threshold,
  percentThreshold,
  percentThresholdLeft,
  ...
  if (hasMetricValues && type) {
    ...
    const priceCommonTitle = isPriceMetric || metric.key ? priceMovingTitle : addressesTitle;

    switch (value) {
      case ETH_WALLETS_OPERATIONS.AMOUNT_DOWN:
        {
          return buildFormBlock(`Balance ${ofTarget}`, 'decreases by ' + threshold);
        }

      case ETH_WALLETS_OPERATIONS.AMOUNT_UP:
        {
          return buildFormBlock(`Balance ${ofTarget}`, 'increases by ' + threshold);
        }

      case PRICE_CHANGE_TYPES.MOVING_DOWN:
        {
          return buildFormBlock(priceCommonTitle, `down ${percentThreshold || 0}% compared to ${timeWindow} ${timeWindowUnitLabel.toLowerCase()} earlier`);
        }

      case PRICE_CHANGE_TYPES.MOVING_UP:
        {
          return buildFormBlock(priceCommonTitle, `up ${percentThreshold || 0}% compared to ${timeWindow} ${timeWindowUnitLabel.toLowerCase()} earlier`);
        }

      case PRICE_CHANGE_TYPES.PERCENT_SOME_OF:
        {
          return buildFormBlock(isPriceMetric ? `${titleTarget} ${ofTarget}` : addressesTitle, `moving up ${percentThresholdLeft || 0}% or moving down ${percentThresholdRight || 0}% compared to ${timeWindow} ${timeWindowUnitLabel.toLowerCase()} earlier`);
        }

      case PRICE_CHANGE_TYPES.ABOVE_OR_EQUAL:
        {
          return buildFormBlock(priceGoesTitle, 'above or equal ' + getUsd(absoluteThreshold, isPriceMetric));
        }

      case PRICE_CHANGE_TYPES.BELOW_OR_EQUAL:
        {
          return buildFormBlock(priceGoesTitle, 'below or equal ' + getUsd(absoluteThreshold, isPriceMetric));
        }

      case PRICE_CHANGE_TYPES.ABOVE:
        {
          return buildFormBlock(priceGoesTitle, 'above ' + getUsd(absoluteThreshold, isPriceMetric));
        }

      case PRICE_CHANGE_TYPES.BELOW:
        {
          return buildFormBlock(priceGoesTitle, 'below ' + getUsd(absoluteThreshold, isPriceMetric));
        }

      case PRICE_CHANGE_TYPES.INSIDE_CHANNEL:
        {
          return buildFormBlock(priceGoesTitle, 'between ' + getUsd(absoluteBorderRight, isPriceMetric) + ' and ' + getUsd(absoluteBorderLeft, isPriceMetric));
        }

      case PRICE_CHANGE_TYPES.OUTSIDE_CHANNEL:
        {
          return buildFormBlock(priceGoesTitle, 'outside ' + getUsd(absoluteBorderRight, isPriceMetric) + ' and ' + getUsd(absoluteBorderLeft, isPriceMetric));
        }
...
export const getNewDescription = newValues => {
  ...
  if (!targetsHeader || Array.isArray(targetsHeader) && targetsHeader.length === 0) {
    return '';
  }

  let metricsHeaderStr = Object.values(titleMetricValuesHeader(true, newValues, `of ${targetsHeader}`)).join(' ').toLowerCase();
  ...
  const repeatingBlock = isRepeating ? `every ${frequencyTimeValue.label} ${frequencyTimeType.label}` : 'only once';
  const channelsReadable = channels.reduce((acc, ch) => {
    return typeof ch === 'object' ? acc.concat(Object.keys(ch)) : acc.concat(ch.toLowerCase());
  }, []);
  const channelsBlock = channelsReadable && channelsReadable.length ? `via ${channelsReadable.join(', ')}` : '';
  return clipText(`Notify me when the ${metricsHeaderStr}. Send me notifications ${repeatingBlock.toLowerCase()} ${channelsBlock}.`, MAX_DESCR_LENGTH);
};
export const buildSignal = (metric, type, slug, Values, selector = 'slug') => {
  const formProps = _objectSpread(_objectSpread({}, METRIC_DEFAULT_VALUES[metric]), Values);
...
export const buildValueChangeSignal = (slug, value, type, metric, selector) => {
  const resultType = type === PRICE_CHANGE_TYPES.ABOVE ? _objectSpread({}, PRICE_ABS_CHANGE_ABOVE) : _objectSpread({}, PRICE_ABS_CHANGE_BELOW);
  return buildSignal(PRICE_ABSOLUTE_CHANGE, resultType, slug, {
    absoluteThreshold: value,
    metric
  }, metric.key === 'price_usd' ? 'slug' : selector);
};
export const buildPercentUpDownSignal = (slug, metric = PRICE_METRIC, selector) => {
  return buildSignal(PRICE_PERCENT_CHANGE, PRICE_PERCENT_CHANGE_ONE_OF_MODEL, slug, {
    metric,
    signalType: {
      label: 'Assets',
      value: 'assets'
...
export const buildDAAPercentUpDownSignal = slug => {
  return buildSignal(PRICE_PERCENT_CHANGE, PRICE_PERCENT_CHANGE_UP_MODEL, slug, {
    metric: DAILY_ACTIVE_ADDRESSES_METRIC,
    signalType: {
      label: 'Assets',
      value: 'assets'
...
export const skipHistoricalPreview = ({
  settings
}) => {
  const {
    target,
  ...
  return getCheckingMetric(settings) === TRENDING_WORDS || target.watchlist_id || typeof target.slug !== 'string' || !time_window;
};
export const getSlugFromSignalTarget = ({
  settings
}) => {
  const {
    target: {
