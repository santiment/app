...
export const filterByChannels = (signals, type) => signals.filter(({
  settings: {
    channel
  }
}) => Array.isArray(channel) ? channel.indexOf(type) !== -1 : channel === type);
export const filterByIsActiveFlag = (signals, statusFilter) => signals.filter(({
  isActive
}) => {
  switch (statusFilter) {
    case 'enabled':
