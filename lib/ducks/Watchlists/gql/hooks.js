...
import { PROJECTS_WATCHLIST_QUERY, WATCHLIST_VOTES_MUTATION, WATHLIST_ITEMS_QUERY } from '../../../queries/WatchlistGQL';
...
export function useWatchlist({
  ...
  const {
    ...
    loading,
    error
  } = useQuery(PROJECTS_WATCHLIST_QUERY, {
    skip: !id || skip,
    variables: {
      id: +id
    }
...
export function useWatchlistVoteMutation({
  id
}) {
  const [vote, {
    data
  }] = useMutation(WATCHLIST_VOTES_MUTATION, {
    refetchQueries: [{
...
export function useWatchlistItems(id) {
  ...
  return useMemo(() => {
    return [data ? data.watchlist.listItems.map(({
      project: {
        slug
      }
    }) => slug) : undefined, loading, error];
  }, [data, loading, error]);
}
export function useRecentWatchlists(watchlistsIDs) {
  ...
  useEffect(() => {
    ...
    let watchlists = [];
    let race = false;
    Promise.all(watchlistsIDs.map((id, i) => getRecentWatchlist(id).then(watchlist => watchlists[i] = watchlist))).then(data => {
      if (race) return;
...
export const getAssetsByFunction = (fn, query) => client.query({
  fetchPolicy: 'network-only',
  query,
  variables: {
    fn: stringifyFn(fn)
