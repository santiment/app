...
import { percentValueFormatter, percentServerValueFormatter, mvrvFormatter, mvrvServerFormatter } from '../formatters';
import { DEFAULT_TIMERANGES } from './timeranges';
import { AGGREGATIONS_LOWER } from './aggregations';
import { formatNumber } from '../../../../../utils/formatting';
import { ETH_SPENT_CELL, MARKET_SEGMENTS_CELL, RANK_CELL, EXCHANGES_CELL } from '../../Table/Columns/columns';
...
const HINTS = {
  mvrv: [{
    ...
    description: 'Average ROI is below 0% (breakeven) for addresses that acquired tokens in the past 1 year'
  }, {
    label: '> 50%',
    firstThreshold: 50,
...
export const Metric = {
  price_usd: {
    ...
    badge: '$',
    withChart: true,
    tableColumnFormatter: value => formatNumber(value, {
      currency: 'USD'
    })
  },
  price_btc: {
  ...
  market_segments: {
    ...
    Widget: props => /*#__PURE__*/React.createElement(MarketSegments, props)
  },
  social_volume_total: {
  ...
  traded_on_exchanges: {
    ...
    isStatic: true,
    Cell: EXCHANGES_CELL,
    Widget: props => /*#__PURE__*/React.createElement(Exchanges, props)
  }
};
Object.keys(Metric).forEach(key => {
  Metric[key].key = key;
});
export const metrics = [Metric.price_usd, Metric.price_btc, Metric.price_volatility_1d, Metric.price_volatility_1w, Metric.price_volatility_2w, Metric.price_volatility_4w, Metric.marketcap_usd, Metric.rank, Metric.eth_spent, Metric.volume_usd, Metric.dev_activity_1d, Metric.daily_active_addresses, Metric.transaction_volume, Metric.transaction_volume_usd, Metric.circulation, Metric.circulation_180d, Metric.circulation_usd_180d, Metric.circulation_180d_usd, Metric.network_growth, Metric.exchange_inflow, Metric.exchange_inflow_usd, Metric.exchange_outflow, Metric.exchange_outflow_usd, Metric.exchange_balance, Metric.mvrv_usd, Metric.mvrv_usd_7d, Metric.mvrv_usd_30d, Metric.mvrv_usd_60d, Metric.mvrv_usd_90d, Metric.mvrv_usd_180d, Metric.mvrv_usd_365d, Metric.dormant_circulation_365d, Metric.bitmex_perpetual_funding_rate, Metric.market_segments, Metric.social_volume_total, Metric.social_dominance_total, Metric.sentiment_volume_consumed_total, Metric.sentiment_balance_total, Metric.mean_dollar_invested_age, Metric.percent_of_total_supply_on_exchanges, Metric.dex_traders_to_cexes_flow, Metric.dex_traders_to_dexes_flow, Metric.dex_traders_to_defi_flow, Metric.dex_traders_to_whale_flow, Metric.dex_traders_to_other_flow, Metric.dex_to_cexes_flow, Metric.dexes_to_defi_flow, Metric.dexes_to_whale_flow, Metric.dexes_to_dex_traders_flow, Metric.dexes_to_other_flow, Metric.cexes_to_dex_flow, Metric.cexes_to_defi_flow, Metric.cexes_to_whale_flow, Metric.cexes_to_dex_traders_flow, Metric.cexes_to_other_flow, Metric.defi_to_cexes_flow, Metric.defi_to_dexes_flow, Metric.defi_to_whale_flow, Metric.defi_to_dex_traders_flow, Metric.defi_to_other_flow, Metric.whale_to_cexes_flow, Metric.whale_to_dexes_flow, Metric.whale_to_defi_flow, Metric.whale_to_dex_traders_flow, Metric.whale_to_traders_flow, Metric.whale_to_other_flow, Metric.other_to_cexes_flow, Metric.other_to_dexes_flow, Metric.other_to_defi_flow, Metric.other_to_whale_flow, Metric.other_to_dex_traders_flow, Metric.other_to_traders_flow, Metric.traders_to_whale_flow, Metric.traders_to_other_flow, Metric.age_consumed, Metric.network_profit_loss, Metric.whale_transaction_count_100k_usd_to_inf, Metric.whale_transaction_count_1m_usd_to_inf, Metric.traded_on_exchanges];
...
export function getActiveBaseMetrics(filter) {
  const activeMetrics = new Set(filter.map(({
    args: {
      metric
    },
    name
...
export function getBaseMetric(metric) {
  ...
  const baseMetricKey = transformedMetricIndex === -1 ? metric : metric.substring(0, transformedMetricIndex);
  return Metric[baseMetricKey] || MetricAlias[baseMetricKey];
}
export function getTimeRangesByMetric(baseMetric, availableMetrics = []) {
  const baseMetricKeyWithSuffix = `${baseMetric.percentMetricKey || baseMetric.key}${METRIC_PERCENT_SUFFIX}`;
  const percentMetrics = availableMetrics.filter(metric => metric.includes(baseMetricKeyWithSuffix));
  const timeRanges = percentMetrics.map(metric => metric.replace(baseMetricKeyWithSuffix, EMPTY_STR));
  return DEFAULT_TIMERANGES.filter(({
    type
  }) => timeRanges.includes(type));
}