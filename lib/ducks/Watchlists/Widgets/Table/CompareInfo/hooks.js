...
const hashAssets = assets => assets.reduce((acc, {
  slug
}) => acc + slug, '');
...
const getIntersection = (source, target) => source.length > 0 ? source.filter(value => target.includes(value)) : target;
...
export function useAvailableMetrics(assets) {
  ...
  useEffect(() => {
    ...
    assets.forEach(asset => {
      ...
      const request = fetchData(PROJECT_METRICS_BY_SLUG_QUERY, {
        slug
