...
import { CURRENT_BALANCE_CELL, BALANCE_CHANGE_CHART_CELL, BALANCE_CHANGE_PERCENT_CELL, CATEGORIES, NOTE_COLUMN, LABELS_COLUMN, ASSETS_DISTRIBUTION_COLUMN } from '../../../../WatchlistAddressesTable/columns';
...
export const buildColumns = (baseMetrics, allMetrics, restrictedMetrics) => {
  ...
  baseMetrics.forEach(baseMetric => {
    ...
    if (isStatic) {
      Column[key] = _objectSpread(_objectSpread({}, baseMetric), {}, {
        disableSortBy: true,
        Header: label
      });
      return;
...
export const getColumns = (columnsKeys, type) => columnsKeys.map(key => type === BLOCKCHAIN_ADDRESS ? AddressColumn[key] : Column[key]).filter(Boolean);