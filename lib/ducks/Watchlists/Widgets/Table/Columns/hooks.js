...
import { ASSETS_DISTRIBUTION_COLUMN, LABELS_COLUMN, NOTE_COLUMN } from '../../../../WatchlistAddressesTable/columns';
export function useCategories(type) {
  ...
  const addressesCategories = useMemo(() => {
    if (type === BLOCKCHAIN_ADDRESS) {
      ...
      return {
        General: {
          _: [LABELS_COLUMN, NOTE_COLUMN, ASSETS_DISTRIBUTION_COLUMN].map(item => ({
            item
          }))
        },
        Assets: {
          _: allAssetColumns.map(item => ({
            item
          }))
        }
      };
...
export const useManipulateColumns = () => {
  ...
  function toggleColumn(columnKey, isActive) {
    const newActiveKeys = isActive ? addKey(columnKey) : activeKeys.filter(key => key !== columnKey);
