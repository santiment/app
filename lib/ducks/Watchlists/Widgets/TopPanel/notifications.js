...
export const notifyCreation = (title, link) => dispatchNotification({
  variant: 'success',
...
export const notifyError = (title, action) => dispatchNotification({
  variant: 'error',
  title: `Couldn't ${action} the ${title}. Please, contact our support`,
  dismissAfter: 5000
});
export const notifyDeletion = name => dispatchNotification({
  variant: 'success',
...
export function notifyLoginForSave() {
  return dispatchNotification({
    ...
    description: "Your settings will be lost after refresh if you're not logged in to Sanbase",
    dismissAfter: 8000,
    actions: [{
      label: 'Log in',
...
export function notifyMonitoring({
  type = 'watchlist',
  name,
  isMonitored
}) {
  return dispatchNotification({
    variant: 'success',
    title: isMonitored ? `You are monitoring "${name}" ${type} now` : `You won't receive reports with "${name}" ${type}`
  });
}