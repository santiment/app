...
function formatFrequencyStr(cooldown) {
  const {
    amount: cooldownCount,
    format: cooldownPeriod
  } = parseIntervalString(cooldown || '5m');
...
export function getDescriptionStr({
  cooldown,
  channels,
  isRepeating
}) {
...
function formatConditionToCompared({
  ...
  let formattedCondition = hasPriceIcon ? condition : condition.replaceAll('$', isPercentIcon ? '%' : '');
...
function validateNotificationsAndTitle({
  ...
  if (onlyValidate && stepIndex > notificationsStepIndex) {
    ...
    if (settings.channel.length > 0 && settings.channel.some(item => typeof item !== 'string')) {
      const telegramChannel = settings.channel.find(item => typeof item !== 'string' && 'telegram_channel' in item);
...
function validateWalletStep({
  ...
  if (stepIndex > 0 && onlyValidate || !onlyValidate) {
    if (settings.type === 'wallet_movement' && (!settings.target.address || !settings.selector.infrastructure || !settings.selector.slug)) {
      invalidSteps.push('wallet');
    }

    if ((settings.type === 'wallet_usd_valuation' || settings.type === 'wallet_assets_held') && (!settings.target.address || !settings.selector.infrastructure)) {
      invalidSteps.push('wallet');
    }
  }
...
export function getMetricSignalKey(minInterval) {
  ...
  const {
    amount,
    format
  } = parseIntervalString(minInterval);
