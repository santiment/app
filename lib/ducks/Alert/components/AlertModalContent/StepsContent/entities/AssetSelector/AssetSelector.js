...
const AssetSelector = ({
  ...
  const [, {
    ...
    setValue: setSlug
  }] = useField(isWords ? 'settings.target.word' : 'settings.target.slug');
  const [,, {
    setValue: setMetric
  ...
  useEffect(() => {
    if (value && value.length > 0 && projects && projects.length > 0) {
      ...
      let assets = typeof value === 'string' ? [isWords ? projects.find(project => project.slug === value) || wordAsset : projects.find(project => project.slug === value)] : value.map(item => {
        const wordAssetItem = words.find(word => word.slug === value) || {
          id: item,
          slug: item,
          name: item
        };
        return isWords ? projects.find(project => project.slug === item) || wordAssetItem : projects.find(project => project.slug === item);
      });
      setListItems([...assets]);
    }
  }, [projects, words]);
  const setSelectedAssets = useCallback(selected => {
    if (listItems.length !== selected.length) {
      if (selected.length === 0) {
        ...
        if (isSocial) {
          track.event(AlertsEvents.SetAlertSocialTrend, {
            trend: trackMetadata
          });
        } else {
          track.event(AlertsEvents.SetAlertAsset, {
            asset: trackMetadata
          });
        }
  ...
  const toggleAsset = useCallback(({
    ...
    if (isAssetInList) {
      const filteredAssets = items.filter(({
        id
      }) => id !== project.id);
  ...
  const filteredProjects = useMemo(() => projects.filter(project => project.name.toLowerCase().includes(searchTerm.toLowerCase()) && !listItemsIds.has(project.id)), [listItems, projects, searchTerm]);
  const filteredWords = useMemo(() => {
    if (!isWords) {
      return [];
    }

    return words.filter(word => word.name.toLowerCase().includes(searchTerm.toLowerCase()) && !listItemsIds.has(word.id));
  }, [listItems, words, searchTerm]);
  const allProjects = useMemo(() => [...listItems, ...filteredWords, ...filteredProjects], [filteredProjects, filteredWords]);
  const filteredListItems = useMemo(() => listItems.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase())), [searchTerm, listItems]);
  ...
  let children = /*#__PURE__*/React.createElement(React.Fragment, null, !isSocial && !isWords && /*#__PURE__*/React.createElement("div", {
    className: styles.titleWrapper
  }, /*#__PURE__*/React.createElement(StepTitle, {
    title: "Select Asset",
    className: styles.title
  }), listItems.length > 0 && /*#__PURE__*/React.createElement(NextStep, {
    label: "Choose Metric",
    onClick: handleNextClick
  })), /*#__PURE__*/React.createElement(Search, {
    type: "text",
    className: cx(styles.search, isWords && styles.searchWithWords),
    placeholder: isWords ? 'Type a word and press Enter or choose from bellow' : 'Search for asset',
    value: searchTerm,
    onChange: value => setSearchTerm(value),
    onKeyPress: handlePressEnter
  }), !isWords && /*#__PURE__*/React.createElement(ProjectsSelectTabs, {
