...
const ChannelsSelector = () => {
  ...
  const webhookObj = useMemo(() => value.find(channel => typeof channel !== 'string' && 'webhook' in channel && channel), [value]);
  const telegramObj = useMemo(() => value.find(channel => typeof channel !== 'string' && 'telegram_channel' in channel && channel), [value]);
  const checkPushAvailability = useCallback(() => {
    navigator.serviceWorker && navigator.serviceWorker.getRegistrations && navigator.serviceWorker.getRegistrations().then(registrations => setIsPushDisabled(!getSanSonarSW(registrations)));
  }, [setIsPushDisabled]);
  useEffect(() => {
    checkPushAvailability();
  }, []);
  useEffect(() => {
    track.event(AlertsEvents.SetAlertChannel, {
      channel: value
    });
  }, [value]);
  const handleChangeChannels = useCallback(channelTitle => () => {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(EmailToggle, {
    ...
    className: styles.divider
  }), /*#__PURE__*/React.createElement(WebhookToggle, {
