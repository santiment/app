...
export const useUpdateFinishedSteps = ({
  ...
  useEffect(() => {
    if (values.title && values.description) {
      !finishedSteps.has(nameAndDescriptionIndex) && setFinishedSteps(prev => [...prev, nameAndDescriptionIndex]);
    } else {
      setFinishedSteps(prev => prev.filter(step => step !== nameAndDescriptionIndex));
    }

    if (values.cooldown && values.settings.channel.length > 0) {
      !finishedSteps.has(nameAndDescriptionIndex - 1) && setFinishedSteps(prev => [...prev, nameAndDescriptionIndex - 1]);
    } else {
    ...
    if (selectedType) {
      switch (selectedType.title) {
        ...
        case 'Screener':
          {
            if (values.settings.operation.selector && values.settings.operation.selector.watchlist_id) {
              !finishedSteps.has(0) && setFinishedSteps(prev => [...prev, 0]);
            } else {
              setFinishedSteps(prev => prev.filter(step => step !== 0));
            }
        ...
        case 'Wallet address':
          {
            if (values.settings.target && values.settings.target.address && values.settings.selector && values.settings.selector.infrastructure) {
              !finishedSteps.has(0) && setFinishedSteps(prev => [...prev, 0]);
            } else {
              setFinishedSteps(prev => prev.filter(step => step !== 0));
            }
