...
import { PERCENT_METRICS, USD_METRICS } from '../components/AlertModalContent/StepsContent/MetricAndConditions/constants';
export const useUpdateNameAndDescription = ({
  ...
  const {
    ...
    skip: !selectedType || selectedType.title !== 'Screener' && selectedType.title !== 'Watchlist' && selectedType.title !== 'Social trends'
  });
  ...
  useEffect(() => {
    if (selectedStep !== nameAndDescriptionIndex && shouldUpdateNameAndDescription && selectedType) {
        ...
        case 'Asset':
          {
            ...
            if (hasSlug) {
              if (typeof slug === 'string') {
                ...
                assets = slug.map(item => projects.find(project => project.slug === item).name).join(', ');
              }
            }
            ...
            if (operation && metric && time_window && hasSlug) {
              ...
              setTitle(`${assets || ''} ${metricLabel ? metricLabel.toLowerCase() : ''} ${conditionsStr || ''}`);
              ...
              if (cooldown && channel.length > 0) {
                ...
                setDescription(`Notify me when the ${metricLabel ? metricLabel.toLowerCase() : ''} of ${assets || ''} ${conditionsStr || ''}. ${notificationsStr}`);
              }
            } else {
              setTitle('');
            }
        ...
        case 'Watchlist':
          {
            ...
            if (operation && metric && time_window && hasWatchlist) {
              setTitle(`${capitalizeStr(watchlistName) || ''} ${metricLabel ? metricLabel.toLowerCase() : ''} ${conditionsStr || ''}`);
              ...
              if (cooldown && channel.length > 0) {
                ...
                setDescription(`Notify me when the ${metricLabel ? metricLabel.toLowerCase() : ''} of ${watchlistName || ''} ${conditionsStr || ''}. ${notificationsStr}`);
              }
            } else {
              setTitle('');
            }
        ...
        case 'Screener':
          {
            ...
            if (hasScreener) {
              ...
              if (cooldown && channel.length > 0) {
                ...
                setDescription(`Notify me when any project enters/exits ${screenerName}. ${notificationsStr}`);
              }
            } else {
              setTitle('');
            }
        ...
        case 'Wallet address':
          {
            ...
            if (hasSlugTicker && address && infrastructure && operation && Object.keys(operation).length > 0) {
              const {
                selectedCount,
                selectedOperation
              } = parseOperation(operation);
              ...
              if (cooldown && channel.length > 0) {
                ...
                setDescription(`Notify me when the balance of ${slugTicker.toLowerCase()} wallet ${address || ''} ${conditionsStr || ''}. ${notificationsStr}`);
              }
              ...
              if (cooldown && channel.length > 0) {
                ...
                setDescription(`Notify me when the balance of wallet ${address || ''} ${conditionsStr || ''}. ${notificationsStr}`);
              }
            } else if (address && infrastructure && !operation) {
              setTitle('Existed assets exit the wallet or new asset enters the wallet with non-zero balance');
              ...
              if (cooldown && channel.length > 0) {
                ...
                setDescription(`Notify me when existed assets exit the wallet or new asset enters the wallet with non-zero balance. ${notificationsStr}`);
              }
            } else {
              setTitle('');
            }
        ...
        case 'Social trends':
          {
            ...
            if (!noSlug && slug && projects.length > 0) {
              assets = typeof slug === 'string' ? projects.find(project => project.slug === slug).name : slug.map(item => projects.find(project => project.slug === item).name).join(', ');
            }
            ...
            if (!noWord && word && projects.length > 0) {
              ...
              assets = typeof word === 'string' ? projectItem.slug : word.map(item => (projects.find(project => project.slug === item) || {
                slug: item
              }).slug).join(', ');
            }
            ...
            if (!noWord && word || !noSlug && slug || hasWatchlist) {
              ...
              if (cooldown && channel.length > 0) {
                ...
                if (!noWord && word || !noSlug && slug) {
                  setDescription(`Notify me when the ${assets} appears in social trends. ${notificationsStr}`);
                }

                if (hasWatchlist) {
                  setDescription(`Notify me when the ${watchlistName} appears in social trends. ${notificationsStr}`);
                }
              }
            } else {
              setTitle('');
