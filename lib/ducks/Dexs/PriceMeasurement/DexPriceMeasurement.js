...
import styles from './DexPriceMeasurement.module.css';
export const DEX_BY_USD = {
  slug: 'multi-collateral-dai',
  label: 'USD'
};
export const DEX_PRICE_SELECTORS = [DEX_BY_USD, {
  slug: 'ethereum',
  label: 'ETH'
}, {
  slug: 'bitcoin',
...
const DexPriceMeasurement = ({
  ...
  const defaultIndex = useMemo(() => {
    return ranges.findIndex(({
      slug
    }) => slug === defaultSelected.slug);
  }, [ranges, defaultSelected]);
  const [sortedByIndex, setSortedByIndex] = useState(defaultIndex);
  const {
    label
  } = DEX_PRICE_SELECTORS[sortedByIndex];
  return /*#__PURE__*/React.createElement("div", {
    className: cx(styles.container, className)
  }, "Metrics measurement:", /*#__PURE__*/React.createElement(Range, {
