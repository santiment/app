...
export const DEXs = ['UniswapV2', 'Sushiswap', 'Balancer', 'dYdX', '0x_v2', 'Curve', '0x_v3', 'KyberNetwork', 'Uniswap', 'Etherdelta', 'Bancor', 'Airswap', 'Gnosis', 'OasisDEX', 'IDEX', 'DDEX', '0x_v1', 'TokenStore', 'DEX.Top', 'Synthetix'];

const NumberOfTradesPerDex = ({
  metrics,
  measurement: strictMeasurement
}) => {
  const {
    measurement,
    setMeasurement
  } = useDexMeasurement(strictMeasurement);
  ...
  const metricsBuilder = useCallback(({
    ...
    if (slug) {
      result.push(_objectSpread(_objectSpread({}, Metric.price_usd), {}, {
        label: `Price (${ticker})`,
        reqMeta: {
          slug: slug
        }
