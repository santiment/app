...
import { checkIsProjectMetricKey, getProjectMetricByKey, getMetricByKey, buildProjectMetricKey } from '../metrics';
...
const convertKeysToMetrics = (keys, dict) => keys && toArray(keys).filter(Boolean).map(getMetricByKey);

export const reduceStateKeys = (State, Data) => Object.keys(State).reduce((acc, key) => {
...
function parseColors(colors = {}, project, SharedKeyIndicator, SharedKeyComparable, SharedKeyMergedHolder) {
  const Colors = {};
  Object.keys(colors).forEach(key => {
    let metricKey = checkIsProjectMetricKey(key) ? key : getMetricByKey(key) && key;

    if (!metricKey) {
      const sharedMetric = SharedKeyIndicator[key] || SharedKeyComparable[key] || SharedKeyMergedHolder[key];
      metricKey = sharedMetric ? sharedMetric.key : buildProjectMetricKey(project, {
        key
      });
    }
...
function extractMergedMetrics(metrics) {
  ...
  for (let i = 0; i < metrics.length; i++) {
    ...
    const mergedMetric = buildMergedMetric(mergedMetricKeys.map(key => HolderDistributionMetric[key]));
...
function parseMetricIndicators(indicators) {
  ...
  Object.keys(indicators || {}).forEach(metricKey => {
    const metric = checkIsProjectMetricKey(metricKey) ? getProjectMetricByKey(metricKey) : getMetricByKey(metricKey);
    MetricIndicators[metric.key] = new Set(indicators[metricKey].map(indicatorKey => {
...
function parseAxesMetrics(axesMetrics, metrics, SharedKeyIndicator, SharedKeyComparable, SharedKeyMergedHolder) {
  ...
  axesMetrics.forEach(key => {
    const metric = parseMetric(key, SharedKeyIndicator, SharedKeyComparable) || SharedKeyMergedHolder[key];
...
export function parseSharedWidgets(sharedWidgets, project) {
  return sharedWidgets.map(({
    ...
    const [holderMetrics, cleanedMetricKeys, SharedKeyMergedHolder] = extractMergedMetrics(metrics);
    const [comparedMetrics, SharedKeyComparable] = parseSharedComparables(comparables);
    const cleanedMetrics = cleanedMetricKeys.map(key => parseMetric(key, SharedKeyIndicator, SharedKeyComparable));
    const parsedMetrics = cleanedMetrics.filter(Boolean).concat(comparedMetrics).concat(holderMetrics);
    const [axesMetricSet, disabledAxesMetricSet] = parseAxesMetrics(axesMetrics, parsedMetrics, SharedKeyIndicator, SharedKeyComparable, SharedKeyMergedHolder);
    return TypeToWidget[widget].new({
      ...
      MetricColor: parseColors(colors, project, SharedKeyIndicator, SharedKeyComparable, SharedKeyMergedHolder),
...
export function translateMultiChartToWidgets(metrics, comparables = []) {
  ...
  return noPriceMetrics.map(metric => ChartWidget.new({
    metrics: priceMetric ? [priceMetric, metric] : [metric]
  })).concat(comparables.map(comparable => ChartWidget.new({
