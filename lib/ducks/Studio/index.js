const _excluded = ["defaultWidgets", "defaultSidepanel", "defaultSettings", "Extensions"];
...
import { HOLDER_DISTRIBUTION_COMBINED_BALANCE_NODE, HOLDER_DISTRIBUTION_NODE, HOLDER_LABELED_DISTRIBUTION_NODE } from './Sidebar/nodes';
import { HolderDistributionCombinedBalanceAbsoluteMetric, HolderDistributionMetric, HoldersLabeledDistributionMetric } from './Chart/Sidepanel/HolderDistribution/metrics';
...
export const Studio = _ref => {
  ...
  function deleteConnectedWidget(connectedWidget, parentWidget) {
    parentWidget.connectedWidgets = parentWidget.connectedWidgets.filter(w => w !== connectedWidget);
    rerenderWidgets();
  }
  ...
  function toggleWidgetMetric(widget, metric) {
    ...
    if (widget.Widget !== HolderDistributionWidget && widget.Widget !== HolderDistributionCombinedBalanceWidget && widget.Widget !== HolderDistributionLabeledWidget && metrics.length === 0) {
      deleteWidget(widget);
    } else {
  ...
  function toggleSelectionMetric(metric, project) {
    const deducedMetric = deduceItems(selectedMetrics, !project || metric.base ? metric : project.slug === settings.slug ? metric : newProjectMetric(project, metric));
  ...
  function onSidebarItemClick(item, project) {
    ...
    if (type === Type.SIDEPANEL) {
      ...
      if (key === HOLDER_DISTRIBUTION_NODE.key || key === HolderDistributionMetric.holders_distribution_1_to_10.key) {
        setWidgets([...widgets, HolderDistributionWidget.new(scrollIntoView)]);
      } else if (key === HOLDER_LABELED_DISTRIBUTION_NODE.key || key === HoldersLabeledDistributionMetric.holders_labeled_distribution_1_to_10.key) {
        setWidgets([...widgets, HolderDistributionLabeledWidget.new(scrollIntoView)]);
      } else if (key === HOLDER_DISTRIBUTION_COMBINED_BALANCE_NODE.key || key === HolderDistributionCombinedBalanceAbsoluteMetric.holders_distribution_combined_balance_1_to_10.key) {
  ...
  function onWidgetClick(widget, appliedMetrics = selectedMetrics, appliedWidgets = selectedWidgets) {
    if (currentPhase === Phase.MAPVIEW) {
      if (widget.chartRef) {
        widget.chartRef.current.canvas.scrollIntoView({
          block: 'center'
        });
        onOverviewClose();
      }
    ...
    widget.connectedWidgets = mergeConnectedWidgetsWithSelected(widget.connectedWidgets, appliedWidgets);
    widget.MetricSettingMap = mergeMetricSettingMap(widget.MetricSettingMap, selectedMetricSettingsMap);
    rerenderWidgets();
    resetSelecion();
  }
  ...
  function onNewChartClick(appliedMetrics = selectedMetrics, scrollIntoViewOnMount) {
    ...
    setWidgets([...widgets, ChartWidget.new({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    isICOPriceActive: isICOPriceActive,
    isOverviewOpened: isOverviewOpened // fn
    ,
  ...
  }), isOverviewOpened && /*#__PURE__*/React.createElement(SelectionOverview, {
    widgets: widgets,
