...
const projectsExtracter = ({
  data: {
    projects
  }
}) => projects;

const watchlistProjectsExtracter = ({
  data: {
    watchlist
  }
}) => watchlist ? watchlist.listItems.map(({
  project
}) => project) : [];
...
function getProjectsByCategory(category, fetchCustomCategory, modifyCategory) {
  ...
  }).then(CategoryDataExtracter[category]).then(modifyCategory || noop).then(normalizeData);
}
...
const ProjectsSelectTabs = ({
  ...
  function onTabSelect(category) {
    getProjectsByCategory(category, CustomCategory[category], CategoryModifier[category]).then(onSelect);
  ...
  return showTabs && /*#__PURE__*/React.createElement(Tabs, {
    options: customTabs,
