...
export const getMetricByKey = key => Metric[key] || CompatibleMetric[key] || searchFromSubmetrics(key) || tryMapToTimeboundMetric(key) || HolderDistributionMetric[key];

const normalizeSlug = slug => '_' + slug.replace(/-/g, DASH_CONNECTOR);

const parseNormalizedSlug = normalizedSlug => normalizedSlug.slice(1).replace(DASH_CONNECTOR_REGEX, '-');

const buildKey = (slug, ticker, metricKey) => `${normalizeSlug(slug)}${METRIC_CONNECTOR}${ticker}${METRIC_CONNECTOR}${metricKey}`;
...
const getController = controller => _extends({}, DEFAULT_CONTROLLER, controller);
...
export function getProjectMetricByKey(key, connector = METRIC_CONNECTOR, controller) {
  ...
  return newProjectMetric({
    slug,
    ticker
  }, metric, isProjectMetricConnector ? key : buildKey(slug, ticker, metricKey));
}
