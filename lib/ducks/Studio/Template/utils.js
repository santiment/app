...
export const getTemplateSharePath = ({
  id,
  title
...
export function prepareTemplateLink(template, asProject) {
  ...
  return `${PATHS.CHARTS}/${getSEOLinkFromIdAndTitle(templateId, title || '')}` + window.location.search;
}
...
export const buildTemplateMetrics = ({
  metrics
...
const getTemplateMetrics = ({
  metrics,
  project
}) => parseTemplateMetrics(metrics, project).map(({
  label
}) => label);
