...
import { getIntervals, getValidInterval, getCandlesMinInterval } from '../Chart/MetricSettings/hooks';
export function useCandleMetricSettings(MetricSettingMap, {
  from,
  to
}) {
  return useMemo(() => {
    MetricSettingMap.forEach((MetricSettings, metric) => {
      ...
      const validInterval = getValidInterval(MetricSettings.interval, getIntervals(getCandlesMinInterval(from, to)));
