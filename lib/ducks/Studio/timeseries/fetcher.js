...
import { SOCIAL_ACTIVE_USERS_TELEGRAM, SOCIAL_ACTIVE_USERS_TWITTER } from '../../dataHub/submetrics';
...
_extends(Fetcher, {
  ...
  topHoldersPercentOfTotalSupply: {
    ...
    preTransform: aliasTransform('topHoldersPercentOfTotalSupply', 'inTopHoldersTotal')
  },
  ethSpentOverTime: {
...
const transformAliases = new Set(['gasUsed', 'historicalBalance', 'topHoldersPercentOfTotalSupply', 'ethSpentOverTime', 'minersBalance']);
export const getQuery = (metric, metricSettings) => {
  ...
  const metricFetcher = withoutRoot && Fetcher[key] ? Fetcher[key] : Fetcher[queryKey];
...
export const fetchData = (query, variables, signal) => client.query({
  query,
  variables,
  context: {
    fetchOptions: {
