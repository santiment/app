...
const IndicatorsSetting = ({
  ...
  const activeIndicators = useMemo(() => new Set(MetricIndicators[metric.key]), [metric, MetricIndicators]);
  const activeLabels = useMemo(() => INDICATORS.filter(indicator => activeIndicators.has(indicator)).map(labelExtractor).join(', '), [activeIndicators]);

  function onToggle(indicator) {
    const indicatorMetric = activeIndicators.has(indicator) ? removeCachedIndicator(metric, indicator) : cacheIndicator(metric, indicator);

    if (indicatorMetric) {
      const {
        interval
      } = getMetricSetting(widget.MetricSettingMap, metric);

      if (interval) {
        widget.MetricSettingMap.set(indicatorMetric, {
          interval
        });
      }
  ...
  return /*#__PURE__*/React.createElement(Dropdown, {
    align: "start",
    trigger: /*#__PURE__*/React.createElement(Setting, null, "Indicators: ", activeLabels)
  }, INDICATORS.map(indicator => /*#__PURE__*/React.createElement(Option, {
    key: indicator.key,
