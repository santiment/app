...
const buildInsightsGetter = (query, variables) => client.query({
  query,
  variables
});

const allInsightsExtractor = ({
  data: {
    insights
  }
}) => insights;
...
export function getFollowingsInsights() {
  ...
  }) => currentUser ? currentUser.following.users.flatMap(({
    insights
  }) => insights).sort(publishDateSorter) : []);
}
export function getFollowingsCount() {
  return client.query({
    query: FOLLOWINGS_COUNT_QUERY
  }).then(({
    data: {
      currentUser
    }
