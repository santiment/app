...
const getDomainGroup = ({
  key,
  domainGroup = key
}) => domainGroup;

const checkIfIsIndicatorOf = ({
  key
}, {
  indicator,
  queryKey
...
const checkIndicators = (metricA, metricB) => metricB.indicator ? checkIfIsIndicatorOf(metricA, metricB) : checkIfIsIndicatorOf(metricB, metricA);
...
export function useMultiAxesMetricKeys(widget, metrics, ErrorMsg = {}, domainGroups) {
  ...
  return useMemo(() => {
    ...
    return result.filter(metric => axesMetricSet.has(metric) && !domainDependencies.has(metric.key)).map(getKey);
...
export function useAxesMetricsKey(metrics, isDomainGroupingActive) {
  return useMemo(() => {
    ...
    let hasSameDomain = isDomainGroupingActive ? mainAxisDomain === getDomainGroup(secondaryAxisMetric) : checkIfAreMirrored(mainAxisMetric, secondaryAxisMetric) || checkIndicators(mainAxisMetric, secondaryAxisMetric);
