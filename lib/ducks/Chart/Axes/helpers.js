import { drawAxes, drawAxesTicks, drawLeftAxis, drawLeftAxisTicks } from '@santiment-network/chart/axes';
...
export const selectYFormatter = metricKey => mirroredMetrics.includes(metricKey) ? value => yFormatter(Math.abs(value)) : TooltipSetting[metricKey] && TooltipSetting[metricKey].axisFormatter || yFormatter;
export function plotAxes(chart, scale) {
  ...
  if (chart.minMaxes[mainAxisMetric]) {
    drawAxesTicks(chart, mainAxisMetric, isDayInterval(chart) ? getDateHoursMinutes : getDateDayMonthYear, selectYFormatter(mainAxisMetric), ticksPaintConfig, scale, xAxesTicks, yAxesTicks);
  }

  if (secondaryAxisMetric && chart.minMaxes[secondaryAxisMetric]) {
    drawLeftAxis(chart, axesColor);
    drawLeftAxisTicks(chart, secondaryAxisMetric, selectYFormatter(secondaryAxisMetric), ticksPaintConfig, scale, 8);
  }
}