import { drawHoverLineX, drawHoverLineY, drawTooltip, drawValueBubbleY, drawValueBubbleX } from '@santiment-network/chart/tooltip';
import { handleMove as handlePointEvent, getHoveredIndex } from '@santiment-network/chart/events';
import { logScale, valueByY, valueByLogY } from '@santiment-network/chart/scales';
import { drawAlertPlus } from './alert';
import { clearCtx, getDateDayMonthYear, getDateHoursMinutes, yBubbleFormatter, isDayInterval } from '../utils';
...
export function plotTooltip(chart, marker, point, event) {
  ...
  axesMetricKeys.forEach((metricKey, i) => {
    ...
    drawValueBubbleY(chart, ctx, yBubbleFormatter(value, metricKey), y, bubblesPaintConfig, i === 0 && chart.isAlertsActive ? 5 : offset);
    offset += 50;
  });
