...
export function plotTooltip(chart, marker, point, options) {
  ...
  const {
    ...
    value: datetime
        ...
        metrics = _objectWithoutProperties(point, _excluded);
  ...
  if (options && options.customTooltip) {
    ...
    if (drawnMetrics.length) {
      ...
      drawValueBubbleY(chart, ctx, yBubbleFormatter(value, tooltipKey), y, bubblesPaintConfig, chart.isAlertsActive ? 5 : 0);
    }
