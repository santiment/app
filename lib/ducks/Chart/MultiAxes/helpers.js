...
import { drawAxes, drawXAxisTicks, drawAxisLine, drawYAxisTicks } from '@santiment-network/chart/axes';
...
function plotMetricLastValueBubble(chart, LastMetricPoint, metricKey, offset, bgColor) {
  ...
  const paintConfig = _extends({}, bubblesPaintConfig, {
    bgColor,
...
function plotXAxis(chart, formatter) {
  ...
  drawXAxisTicks(chart, isDayInterval(chart) ? getDateHoursMinutes : getDateDayMonthYear, scale, xAxesTicks);
