import { HandleType, newLine, checkIsLineHovered, getPressedHandleType, absoluteToRelativeCoordinates } from './helpers';
const LineLockType = {
  FREE: 0,
  X: 1,
  Y: 2
...
const getDprCoordinates = ({
  dpr
}, [x, y]) => [x * dpr, y * dpr];

const getLineLockType = (x1, y1, x2, y2) => Math.abs(x2 - x1) < Math.abs(y2 - y1) ? LineLockType.X : LineLockType.Y;
...
export function handleLineCreation(chart, setSelectedLine, setIsDrawing, setIsNewDrawing) {
  ...
  function onMouseDown(e) {
    ...
    function onMouseMove(e) {
      ...
      const lineLock = e.shiftKey ? getLineLockType(startX, startY, moveX, moveY) : LineLockType.FREE;
