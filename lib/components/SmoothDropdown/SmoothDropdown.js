...
class SmoothDropdown extends Component {
  constructor(...args) {
    ...
    this.modalRoot = document.getElementById('dd-modal');
    this.ddTemplate = document.querySelector('#dd-template');
    this.ddContainer = this.ddTemplate ? this.ddTemplate.cloneNode(true).firstElementChild : null;
  ...
  componentDidMount() {
    if(!this.ddContainer) return
    this.modalRoot.appendChild(this.ddContainer); // GarageInc: HACK for IOS(IPAD and etc) which can't handle mouseLeave/touchCancel/blur events
  ...
  render() {
    ...
    if(!this.ddContainer) return null
