...
const ForceActionRedirector = ({
  ...
  function checkOpenTabs() {
    ...
    channel.join().receive('ok', () => {
      channel.push('open_restricted_tabs', {}, PUSH_TIMEOUT).receive('ok', ({
        open_restricted_tabs
      }) => {
        const showShowModal = open_restricted_tabs > MAX_TABS;
        setShowTabLimitModal(showShowModal);

        if (showShowModal) {
          track.event('tab_limit_modal_showed', {
            open_restricted_tabs
          });
        }
      }).receive('error', () => setTimeout(checkOpenTabs, TRY_WAIT_TIME_MS)).receive('timeout', () => setTimeout(checkOpenTabs, TRY_WAIT_TIME_MS));
    });
  } // Prevents user cheat.
  ...
  return showTabLimitModal ? /*#__PURE__*/React.createElement(TabLimitModal, {
