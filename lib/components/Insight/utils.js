export const noTrendTagsFilter = ({
  name
}) => !name.endsWith('-trending-words');
export const getInsightTrendTagByDate = date => `${date.getUTCDate()}-${date.getUTCMonth()}-${date.getUTCFullYear()}-trending-words`;
export const popularitySort = ({
  ...
  votes: {
    totalVotes: bTotalVotes
  }
}) => {
  const aDate = new Date(aCreatedAt);
  const bDate = new Date(bCreatedAt);
  return aDate.getDate() === bDate.getDate() && aDate.getMonth() === bDate.getMonth() && aTotalVotes > bTotalVotes ? -1 : 1;
};
...
export const publishDateSorter = ({
  publishedAt: a
}, {
  publishedAt: b
}) => new Date(b) - new Date(a);
export const popularitySortReducer = insights => insights.sort(creationDateSort).sort(popularitySort);
export const creationSortReducer = insights => insights.sort(creationDateSort);
export const filterInsightsNoDrafts = ({
  readyState
}) => readyState !== 'draft';
