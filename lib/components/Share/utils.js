...
export const getShortUrl = fullUrl => client.mutate({
  mutation: SHORT_URL_MUTATION,
  variables: {
    fullUrl
  }
}).then(urlExtractor);
export const getFullUrl = shortUrl => client.query({
  ...
  if (pathname.startsWith(SHARE_PATH)) {
    ...
    loop: for (; hashEndIndex < pathname.length; hashEndIndex++) {
      switch (pathname[hashEndIndex]) {
        ...
        case '?':
        case '&':
          // NOTE(vanguard): remove when external link generator is fixed [21 Jan, 22]
          break loop;
    ...
    getFullUrl(pathname.slice(hashStartIndex, hashEndIndex)).then(fullPath => history.push(fullPath)).catch(e => {
      console.error(e);
      history.push('/');
    });
  }
