...
const TOP_CLAIMERS_QUERY = gql`
  query getMetric($from: DateTime!, $to: DateTime!) {
    getMetric(metric: "uniswap_top_claimers") {
      histogramData(selector: { slug: "uniswap" }, from: $from, to: $to, limit: 50) {
        values {
...
const TRANSACTION_VOLUME_PER_ADDRESS_QUERY = gql`
  query transactionVolumePerAddress(
    ...
    transactionVolumePerAddress(addresses: $addresses, from: $from, to: $to, selector: $selector) {
      address
...
`;
export function useTopClaimers({
  from,
  to,
  slug
}) {
  ...
  if (data.getMetric && data.getMetric.histogramData && data.getMetric.histogramData.values) {
    return [data.getMetric.histogramData.values.data || [], loading];
  }

  return [[], loading];
}
export function useUNIBalances({
  from,
  to,
  addresses = []
}) {
  const {
    ...
    loading
  } = useQuery(ADDRESS_BALANCE_CHANGE_QUERY, {
...
export function useUNITransactionVolume({
  from,
  to,
  addresses = []
}) {
  const {
    ...
    loading
  } = useQuery(TRANSACTION_VOLUME_PER_ADDRESS_QUERY, {
    skip: addresses.length === 0,
    variables: {
      addresses,
      to,
