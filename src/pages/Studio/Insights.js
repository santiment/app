...
import {
  getFollowingsCount,
  getMyInsights
} from '../../ducks/Studio/insights/queries'
import ChartInsights from '../../ducks/Studio/Chart/Insights'

export const useInsightsStoreCreator = () =>
  useMemo(newInsightsContextStore, [])
...
const Insights = ({ InsightsStore, widget }) => {
  ...
  useEffect(
    () => {
      ...
      getMyInsights().then(insights =>
        InsightsStore.apply({ hasMyInsights: !!insights.length })
      )
      getFollowingsCount().then(count =>
        InsightsStore.apply({ hasFollowings: !!count })
      )
    },
    [!!store.insight]
  )
