export function findScrollProperties(currentDashboard) {
  const keysArr = currentDashboard.submenu.filter(({ key }) => key)
  const offsets = keysArr.map(({ key }) => {
  ...
  return { offsets, findCurrentTab, tab }
}

export function scrollToCurrentAnchor({ currentDashboard, hash }) {
  const { offsets } = findScrollProperties(currentDashboard)
