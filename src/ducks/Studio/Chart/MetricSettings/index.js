...
const getSettings = ({ key, domainGroup }) => MetricSettings[key] || MetricSettings[domainGroup]
...
const Settings = ({ className, metric, ...props }) => {
  ...
  return (
    <div className={cx(styles.wrapper, className)}>
      ...
      {isExchangeModifiable(metric) && <ExchangeSetting metric={metric} {...props} />}
      {isIndicatorAssignable(metric) && <IndicatorsSetting metric={metric} {...props} />}
      <ShowAxisSetting metric={metric} {...props} />
      {settings &&
        settings.map(({ key }) => {
          const Setting = SettingToComponent[key]
          return Setting ? <Setting key={key} metric={metric} {...props} /> : null
        })}
    </div>
  )
}
