...
const GENERAL_CHARTS = [
  ...
  DetailedMetric.social_volume_4chan,
]
...
const DetailedBlock = ({
  ...
  const charts = useMemo(
    () => {
      ...
      return COMMUNITY_CHARTS.filter(({ key }) => availableMetrics.includes(key))
    },
    [availableMetrics],
    type,
  )
  ...
  const detectedAsset =
    type === 'community' ? allDetectedAssets.get(settings.slug) : linkedAssets.get(settings.slug)
  ...
  return !shouldShowChart || MetricSettingMap.size === 0 || !priceAsset ? null : (
    <>
        ...
        {charts.map((chart) => (
          <div className={styles.chart} key={chart.key}>
            ...
            <Chart
              ...
              metrics={[chart, { ...Metric.price_usd, reqMeta: { slug: priceAsset.slug } }]}
